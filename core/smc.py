...
# ---------------------------------------------------------------------------
# Trade plan — Entry / Stop Loss / Targets, SMC-style
# (prefers the nearest unmitigated Order Block for entry+SL, matching
#  Satya Trading's own risk-management notes: SL below/above the level that
#  "softens" everything beyond it; targets in R-multiples, no need to chase
#  huge RR — 1:1.5 to 1:3 is plenty).
# ---------------------------------------------------------------------------
@dataclass
class TradePlan:
    entry: float
    ...
    rr1: float
    rr2: float
    basis: str   # "Order Block" | "Pivot level"
...
def compute_trade_plan(
    ...
    matching = [
        ob for ob in obs
        if ob.kind == direction and not ob.mitigated and ob.pos <= signal.p2.pos
    ]
    ob = matching[-1] if matching else None  # most recent qualifying OB

    if signal.bias == "BUY":
        entry = ob.top if ob else signal.p2.price
        raw_sl = ob.bottom if ob else signal.p2.price
        sl = raw_sl * (1 - buffer_pct)
        if sl >= entry:
        ...
        entry = ob.bottom if ob else signal.p2.price
        raw_sl = ob.top if ob else signal.p2.price
        sl = raw_sl * (1 + buffer_pct)
        if sl <= entry:
            sl = entry * (1 + buffer_pct * 3)
        risk = sl - entry
    ...
    return TradePlan(
        ...
        risk_points=risk, rr1=rr1, rr2=rr2,
        basis="Order Block" if ob else "Pivot level",
    )
