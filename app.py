...
from core.divergence import detect_divergences, latest_signal_only, find_price_pivots
...
def _cached_fetch(symbol: str, interval: str, period: str):
    return fetch_candles(symbol, interval, period)
...
for label, symbol in symbol_map.items():
    for tf in sel_timeframes:
        interval, period = config.TIMEFRAMES[tf]
        try:
            df, is_live = _cached_fetch(symbol, interval, period)
            ...
            for s in sigs:
                ...
                is_new = s.signal_key not in alerted_keys
                rows.append({
            ...
            rows.append({
                "Symbol": label, "TF": tf, "Type": f"error: {e}", "Bias": "-",
                "Pivot Time": "-", "Price": None, "RSI": None, "SMC": "-", "SMC Tags": "-", "New": "",
            })
        done += 1
